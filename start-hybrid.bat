...
echo Lancement de l'infrastructure Docker (Postgres, Redis, Loom, ES)...
docker-compose up -d postgres redis elasticsearch loom-api
...
start "Celery Worker" cmd /k "cd backend && call .\.venv\Scripts\activate.bat && celery -A config worker -l info"
